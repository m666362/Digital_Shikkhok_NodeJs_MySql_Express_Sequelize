...
exports.create = (req, callback) => {
  const { roll_number, gradeLevelId, institutionId, userId } = req.body;
  const item = { roll_number, gradeLevelId, institutionId, userId };
  Model.create(item)
    .then((data) => {
...
exports.findOne = (id, callback) => {
  Model.findByPk(id, { include: ["user", "institution", "grade_level"] })
    .then((data) => {
      callback(status.Success.OK.code, data, null);
    })
    .catch((error) => {
